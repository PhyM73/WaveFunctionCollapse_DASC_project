...
class Scan_Pattern:
    """ 
    输入一个图片，转化为由N*N的模式组成的矩阵
    遍历该矩阵，记录出现的模式并记录它们的出现频率，如果允许对称、旋转处理，对所有模式做变换得到更多模式。[p1,p2,p3,...],频率用dict记录
    在patterns中记录每种pattern的具体形式，weights与rules中均用某个pattern在patterns中的index表示该pattern，
    因此之后可以直接处理由int组成的状态空间，用int填充一个矩阵，最后再代入patterns中的数据，以减小计算量
    记录所有模式之间的关系(4*T*x的list),4为四个方向，T为模式个数，x为该模式旁边在该方向可以出现的模式的个数
    [[{p1,p2,p3,...}(第0种模式),{...}(1),{...},...](left), [{...},{...},...](right), [...](up), [...](down)]
    类似于图的邻接表表示，但是第三维用set表示，便于rules的构建
    """
    ...
    def __init__(self, image_path, N = 3, LR_Reflect = False, UD_Reflect = False, Rotatable = False):
        ...
        self.matrix = self.trans_image_to_matrix()#由pattern构成的矩阵
    ...
    def build_patterns(self):
        ...
        def Rotated(pattern):
            ...
            def transposition(pattern):#将一个pattern转置
                ...
                return temp
            p1 = transposition(pattern); p2 = LR_Reflect(p1); p3 = UD_Reflect(p1); p4 = UD_Reflect(p2)          
            return [p1, p2, p3, p4]
    ...
    def get_rules(self):
        ...
        if not sum(self.changable):
            ...
            for x in range(self.width):
                for y in range(self.height):
                    for d in range(4):
                        x1 = x + directions[d][0]; y1 = y + directions[d][1]
...
# i = Scan_Pattern(r'image_path', 3, True, True, True)
# patterns, weights, rules, N = i.patterns, i.weights, i.rules, i.N

class Scan_Tile:
    #可以看成Scan_Pattern类N=1的实现，可以作为一个尝试保留
    def __init__(self, image_path):
        self.image = Image.open(image_path)
        self.width = self.image.width
        self.height = self.image.height
        self.matrix = self.trans_image_to_matrix()
...
#tiles, weights, rules = Scan_Tile(r'image_path').get_rules()
